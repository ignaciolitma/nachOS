...
bool CountArgsToSave(int address, unsigned *count)
{
    ...
    do {
        int i;
        for(i = 0; (i<MAX_PAGE_FAULTS) && (!(SafeReadMem(address + 4 * c, 4, &val))); i++);
...
SaveArgs(int address)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        int j;
        for(j = 0; (j<MAX_PAGE_FAULTS) && (!(SafeReadMem(address + i * 4, 4, &strAddr))); j++);
...
WriteArgs(char **args)
{
    ...
    for (unsigned i = 0; i < c; i++){
        int j;
        for(j = 0; (j<MAX_PAGE_FAULTS) && (!(SafeWriteMem(sp + 4 * i, 4, argsAddress[i]))); j++);
        ASSERT(j<MAX_PAGE_FAULTS);
     }

    int i;
    for(i = 0; (i<MAX_PAGE_FAULTS) && (!(SafeWriteMem(sp + 4 * c, 4, 0))); i++);
